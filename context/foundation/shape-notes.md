...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "missing capability + workflow friction"
    - topic: "insight / differentiation"
      decision: "existing tools are editors not generators; no ride-character control; no interesting stops suggested"
    - topic: "primary persona scope"
      decision: "all recreational riders, individual, riding for pleasure"
    - topic: "auth model"
      decision: "anonymous: generate + download GPX; registered (flat): additionally save and revisit routes"
    - topic: "MVP scope"
      decision: "scoped down — route generation + GPX download in v1; ride description narrative deferred to v2"
    - topic: "timeline"
      decision: "2-3 weeks after-hours; mvp_weeks: 3"
  frs_drafted: 12
  quality_check_status: accepted
---

> Seed idea (verbatim): "planner for motorcycle ride where user can specify how long it should be how much curves he want it should be fast or turistic ride it could generate gpx file and describe of ride"

---

## Non-Goals

- **No turn-by-turn navigation.** RideForge generates and exports routes; it is not a GPS navigator. Riders load the GPX into their existing navigation app (Garmin, OsmAnd, Google Maps). Real-time guidance is permanently out of scope, not deferred.
...
1. **What does "fast" vs "touristic" mean concretely?** — Needs a UI definition before launch (e.g. tooltips or label examples). Owner: user. Block: UI clarity required before FR-004 is shippable.
2. **Which routing API supports curviness optimization?** — Must be validated before any code is written. Candidates: GraphHopper (curvature weighting), OpenRouteService (scenic profile), custom OSM-based approach. Owner: user. Block: yes (entire generation pipeline depends on this).
3. **Which POI data source for FR-011?** — OSM Overpass API (free, global), Google Places (paid), Foursquare. Owner: user. Block: no (FR-011 is nice-to-have).
...
Any individual rider — regardless of bike type or country — who rides for pleasure, not for commuting or delivery. They have free time and a bike. They want a ride that matches their mood and energy, not just the shortest path. They are comfortable downloading a GPX file and loading it into their device or navigation app. Their frustration: planning a good ride takes too long, and generic apps don't understand what makes a route fun.
...
- FR-001: Rider can enter a starting location. Priority: must-have
  > Socrates: Counter-argument considered: "starting location alone isn't enough — riders need to specify a region or area to stay within." Resolution: valid gap. A radius/area constraint is not captured in any current FR. Flagged as an Open Question — likely a must-have addition before v1 ships.
...
- FR-004: Rider can set the ride pace style (fast or touristic). Priority: must-have
  > Socrates: Counter-argument considered: "'fast vs touristic' is vague without a definition — riders will interpret it differently." Resolution: kept as must-have; but both terms must be defined in the UI (e.g. via tooltip or label examples) before launch. Captured as an Open Question for definition.

- FR-005: Rider can trigger route generation from their inputs. Priority: must-have
  > Socrates: Counter-argument considered: "a single Generate button with no feedback is a dead end if generation is slow or fails." Resolution: kept as must-have; visible loading state and error state are required companions. Flagged for Non-Functional Requirements (visible feedback during any operation > 2s).
...
### Primary
- A rider enters a starting location, ride duration, curviness level, and pace (fast vs touristic); hits "Generate"; sees a route drawn on a map; and downloads a GPX file that loads correctly in standard apps (Garmin, Google Maps, OsmAnd).

### Secondary
- Logged-in riders can save generated routes and revisit past rides from their account.

### Guardrails
- The generated GPX file must be valid and loadable in Garmin, Google Maps, and OsmAnd — a broken GPX is a hard regression.
