...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "Claims team processors — highest volume of mixed inbound documents"
    - topic: "document mix"
      decision: "Mostly scanned paper / photos — OCR/ICR is the critical path"
    - topic: "why now"
      decision: "AI/LLM capability just crossed the reliability bar for document understanding"
    - topic: "auth model"
      decision: "Company SSO login; two roles: Operator and Supervisor"
    - topic: "MVP scope"
      decision: "Scoped to one document type (FNOL / insurance claim) end-to-end; ~3 weeks"
    - topic: "OCR/ICR in v1"
      decision: "Nice-to-have; AI agents extract data directly from documents in v1"
    - topic: "supervisor escalation button"
      decision: "Nice-to-have FR added (FR-012)"
    - topic: "non-claim handling"
      decision: "Non-claim documents routed to manual review queue with explicit flag"
    - topic: "routing output"
      decision: "Document + extracted data sent to downstream system AND human team notified"
    - topic: "classification inputs"
      decision: "Document content + file metadata (filename, source mailbox, sender)"
  frs_drafted: 12
...
### Primary
A single FNOL / insurance claim document travels the full pipeline without human intervention until the review step: file is picked up from Azure Blob Storage, the categorizer identifies it as a claim, the claims extraction agent pulls structured data, the operator sees the result in the application, edits if needed, approves, and the document + data is forwarded to the claims department. End-to-end with no silent failures.
...
- **Given** a claims processor is logged in and the system has finished processing an FNOL document picked up from Azure Blob Storage
...
### Document Ingestion
- FR-001: System can monitor and pick up incoming files from Azure Blob Storage. Priority: must-have
  > Socrates: Counter-argument considered: none. Automatic blob pickup is core to the value proposition — without it the operator still manually triggers processing. Stands as written.
...
- Processed documents are available for operator review within 1 hour of landing in Azure Blob Storage, measured from file arrival to appearance in the operator queue.
- All document content and extracted PII is processed and stored within the EU (or applicable local jurisdiction). No document content leaves the company's regulatory boundary for AI processing.
...
Entry: Company SSO login (e.g., Azure AD / Okta). Operators authenticate with corporate credentials — no separate account creation.
...
- **No custom AI model training or fine-tuning in v1.** Classification and extraction rely on existing LLM/vision APIs as-is. Custom model investment requires accuracy baselines that don't exist yet — this is a v2+ decision.
...
All six greenfield gates passed on 2026-05-25. No gaps.

- Access Control: present — SSO + Operator/Supervisor roles defined
- Business Logic: present — one-sentence classification + routing rule
- Project artifacts: present — shape-notes.md with valid checkpoint
- Timeline-cost ack: present — mvp_weeks: 3 (within 3-week threshold)
- Non-Goals: present — no custom model training, no multi-tenant
